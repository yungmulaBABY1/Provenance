...
|---|---|---|---|
| FAIL- | 43 | 10 | 33 |
| TOOL- | 4 | 1 (TOOL-002 and TOOL-004 open) | 2 |
| GUARD- | 8 | 1 | n/a |

Ten regression cases now exist: case 11 (media acquittal, built), CE-006/007 (joint
citation, built), MIX-01 (mixed control, built), the compliance fixture set
(FAIL-CN-001, run at three burden levels), FAIL-MB-017 (Ceuta prominence, built),
FAIL-MB-018 (Ceuta semantic equivalence, built), FAIL-CA-003 (Shen internal-validity
handoff, PASS on the lead-superset packet), and FAIL-CA-004 (Shen directional-defense
symmetry, PASS), the Shen revised-output checker regression (FAIL-CN-002, built and
failing before repair as intended), and the GalorЅzak cross-prosecutor-leak case
(FAIL-IV-001/002, relative oracle built; paired source artifacts not yet copied into the
fixture).
...
FAIL-CA-003 and FAIL-CA-004 both pass the 2026-08-11 repaired Shen counter-audit:
internal validity was supplied and consumed, all four operator leads were explicit,
and CV-7 became DIRECTIONALLY-CONTESTED without a severity reduction. FAIL-IV-001 and
FAIL-IV-002 are applied but await their paired regression rerun. The remaining unresolved
entries are excluded from the "Never re-observed since fix" count.

**Repository integration note:** the FAIL-MB-017 / FAIL-MB-018 PASS evidence refers
to the integrated 3,232-line RC2 artifact used for the 2026-08-09 rerun (Git blob
`d5f3502fc3ab1af083c12be0a0b1440cbf2adf6f`). Those execution blocks are now in
the canonical prompt. Current `main` renamed that prompt to
`prompts/media-analysis/2-network-coverage.txt` and added the TOOL-004 instrument
preflight; the resulting 3,340-line artifact has Git blob
`9380b619fd16b4212dabec28971f1c6a6b3d18b6`. The TOOL-004 guard has not yet been
exercised by its dedicated two-version regression case.
...
quoted excerpts alone. TOOL-002 and TOOL-004 are open. TOOL-003 was found by inspection
rather than by a run, and has no committed regression case.
...
**Case:** [ceuta-prominence-regression](tests/cases/case_FAIL-MB-017_ceuta-prominence-regression.md).
...
**Case:** [ceuta-semantic-equivalence](tests/cases/case_FAIL-MB-018_ceuta-semantic-equivalence.md).
...
### FAIL-CN-002  -  Canonical verification vocabulary was replaced wholesale and the checker passed
...
**Observed:** the first `check-citation-network-output.py` tested presence with `heading not
in text`  -  a substring search over the whole document. But the COMPLETION LEDGER
