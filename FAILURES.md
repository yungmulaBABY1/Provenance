...
**Evidence.** Entries record the observed behavior directly, quoted where
possible. Entries 001–016 predate the practice of archiving runs and are
supported by those quoted excerpts alone — the originating runs were not
retained and cannot be reconstructed. From FAIL-CE-011 / FAIL-CA-003 /
FAIL-MB-008 onward, save the run that exposed the failure at the time it is
logged, as `tests/evidence/<FAIL-ID>-run.md`, and reference it from the entry.
Cheap while fresh, impossible retroactively.
...
**Only one regression case exists** (case 11), and no originating runs are
archived for entries 001–016. Those entries are protected by quoted excerpts
and the changelog narrative. That is thin: a rule with an observed failure, no
regression case, and no retained run can be neither safely deleted nor safely
kept without re-testing it. See tests/README.md for the case backlog.
