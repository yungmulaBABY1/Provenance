...
Five regression cases now exist: case 11 (media acquittal, built), CE-006/007 (joint
citation, built), MIX-01 (mixed control, built), the compliance fixture set
(FAIL-CN-001, run at three burden levels), and the GalorЅzak cross-prosecutor-leak case
(FAIL-IV-001/002, oracle built; paired source artifacts not yet copied into the fixture).

FAIL-MB-008 and FAIL-MB-009 have an applied but untested per-fact repair. FAIL-MB-010 has a structurally applied MEDIA-P1 repair but no comparative rerun. FAIL-CV-003 and FAIL-CV-004 have applied or structurally validated retrieval gates without a fresh prompt rerun. FAIL-MB-016 remains proposed and untested. FAIL-IV-001 and FAIL-IV-002 are applied but await the paired regression rerun. These eight are excluded from the "Never re-observed since fix" count.
...
**Status:** TESTED. The automatic downgrade has fired at least six times: both Ceuta run-2
audits, all three genuine fixed-packet runs (DeepSeek, Grok, Sol), and at least one run-1
audit. The fixed-packet DeepSeek and Grok runs declared PRIOR SOURCE: NONE; Sol declared a
MODEL-GENERATED prior. The mechanism therefore fires independently of whether a prior was
supplied or generated. Five consecutive zeros under the prior discretionary version. This
closes the TODO question "does the mechanical downgrade actually fire."
...
**Case:** none. The fixed Ceuta packet is the natural fixture but has not yet been encoded
as a formal regression case.

**Status:** initial repair re-tested and shown incomplete. Superseding per-fact rewrite
APPLIED, UNTESTED.
...
**Case:** none. Strong candidate: the three fixed-packet runs.

**Status:** OBSERVED and REPRODUCED. Per-fact repair APPLIED, UNTESTED.
...
**Case:** none. Natural fixture: the fixed Ceuta packet.

**Status:** structural repair applied and parity-checked across both media prompts;
not yet exercised in a comparative live rerun.
